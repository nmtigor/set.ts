...
function _addIssue(context, label, dataset, config2, other) {
  ...
  const message2 = other?.message ?? context.message ??
    getSpecificMessage(context.reference, issue.lang) ??
    (isSchema ? getSchemaMessage(issue.lang) : null) ?? config2.message ??
...
function _isValidObjectKey(object2, key) {
  return Object.hasOwn(object2, key) && key !== "__proto__" &&
  ...
  return {
    ...
    expects: `!${
      _joinExpects(
        requirement.map(
          (value2) =>
            value2 instanceof Date ? value2.toJSON() : _stringify(value2),
        ),
        "|",
      )
    }`,
    ...
    "~run"(dataset, config2) {
      if (
        dataset.typed && this.requirement.some(
...
function partialCheck(paths, requirement, message2) {
  return {
    ...
    "~run"(dataset, config2) {
      if (
        (dataset.typed || _isPartiallyTyped(dataset, paths)) && // @ts-expect-error
        !this.requirement(dataset.value)
      ) {
        _addIssue(this, "input", dataset, config2);
      }
  ...
  return {
    ...
    "~run"(dataset, config2) {
      action({
        ...
        config: config2,
        addIssue: (info) =>
          _addIssue(this, info?.label ?? "input", dataset, config2, info),
      });
      return dataset;
    },
  };
  ...
  return {
    ...
    "~run"(dataset, config2) {
      ...
      return outputDataset.issues
        ? { typed: true, value: getFallback(this, outputDataset, config2) }
        : outputDataset;
    },
  };
}
      ...
      if (this.options.length) {
        ...
        const optionDatasets = await Promise.all(
          this.options.map((schema) =>
            schema["~run"]({ value: input }, config2)
          ),
        );
  ...
  return {
    ...
    async "~run"(dataset, config2) {
      ...
      if (input && typeof input === "object") {
        ...
        dataset.value = {};
        const datasets = await Promise.all(
          Object.entries(input).filter(([key2]) =>
            _isValidObjectKey(input, key2)
          ).map(
            ([entryKey, entryValue]) =>
              Promise.all([
                entryKey,
  ...
  return {
    ...
    "~run"(dataset, config2) {
      ...
      if (Array.isArray(input)) {
        ...
        if (
          !(dataset.issues && config2.abortEarly) &&
...
function partial(schema, keys) {
  const entries2 = {};
  for (const key in schema.entries) {
    entries2[key] = !keys || keys.includes(key)
...
function required(schema, arg2, arg3) {
  ...
  for (const key in schema.entries) {
    entries2[key] = !keys || keys.includes(key)
      ? nonOptional(schema.entries[key], message2)
      : schema.entries[key];
  }
  return {
    ...schema,
...
function requiredAsync(schema, arg2, arg3) {
  ...
  for (const key in schema.entries) {
    entries2[key] = !keys || keys.includes(key)
      ? nonOptionalAsync(schema.entries[key], message2)
      : schema.entries[key];
  }
  return {
    ...schema,
